...
def _sanitize_invalid_tls_env() -> None:
    """Unset broken TLS CA env vars so requests can fall back to system certs."""
    for env_name in ("REQUESTS_CA_BUNDLE", "SSL_CERT_FILE", "CURL_CA_BUNDLE"):
        path = (os.getenv(env_name) or "").strip()
        if not path:
            continue
        if not Path(path).is_file():
            logging.warning("ignore invalid %s=%s", env_name, path)
