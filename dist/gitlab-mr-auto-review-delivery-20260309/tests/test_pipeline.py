...
class StubFeishuClientFailure:
    def __init__(self, events):
        self.events = events

    def publish_markdown_doc(self, markdown, title):
        self.events.append("feishu_doc")
        raise RuntimeError("invalid tls cert bundle path")


def test_run_once_marks_processed_after_review_even_if_downstream_delivery_fails(tmp_path):
    mr_target = _mr(iid=3, sha="sha-3")
    state = JsonStateStore(tmp_path / "state.json")

    reviewer = StubReviewer()
    gitlab = StubGitLabClient([mr_target])
    sohu = StubSohuClient(events=gitlab.events)
    feishu = StubFeishuClientFailure(events=gitlab.events)
    ...
    first_results = pipeline.run_once()

    assert len(first_results) == 1
    assert first_results[0].status == "failed"
    assert state.is_processed(mr_target.unique_key)
    assert reviewer.called == [mr_target.unique_key]

    second_results = pipeline.run_once()
